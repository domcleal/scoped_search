...
ScopedSearch::RSpec::Database.test_databases.each do |db|
  ...
  describe ScopedSearch, "using a #{db} database" do
    ...
    before(:all) do
      ...
      @parent_class = ScopedSearch::RSpec::Database.create_abstract_model do |klass|
        klass.scoped_search on: :int
      end
      @subclass1 = ScopedSearch::RSpec::Database.create_model({int: :integer}, @parent_class)
      @subclass2 = ScopedSearch::RSpec::Database.create_model({int: :integer, related_id: :integer}, @parent_class) do |klass|
    ...
    after(:all) do
      ...
      ScopedSearch::RSpec::Database.drop_model(@subclass1)
      ScopedSearch::RSpec::Database.drop_model(@subclass2)
      ScopedSearch::RSpec::Database.drop_model(@related_class)
      ScopedSearch::RSpec::Database.close_connection
    end

    context 'querying abstract class' do
      it "should fail to search" do
        lambda { @parent_class.search_for('int = 9') }.should raise_error(ScopedSearch::QueryNotSupported, "Class #{@parent_class.name} cannot be queried as it is abstract")
      end

      it "should fail to autocomplete" do
        lambda { @parent_class.complete_for('') }.should raise_error(ScopedSearch::QueryNotSupported, "Class #{@parent_class.name} cannot be queried as it is abstract")
      end
    end

    context 'querying parent definition on concrete subclasses' do
      ...
      it "should autocomplete int, other_int, related_int fields on other subclass" do
    ...
    context 'querying definition on concrete subclass' do
